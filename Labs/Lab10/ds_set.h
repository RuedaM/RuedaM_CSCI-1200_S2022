...
public:
  ...
  iterator find2(const T& key_value) { return find2(key_value, root_); }
...
/*------------------------------------------------------------*/
  TreeNode<T>*  copy_tree(TreeNode<T>* old_root) {
    TreeNode<T>* newTree = NULL;

    if (old_root!=NULL) {
      newTree = new TreeNode<T>;
      newTree->value = old_root->value;
      newTree->left = copy_tree(old_root->left);
      newTree->right = copy_tree(old_root->right);
    }
...
/*------------------------------------------------------------*/
  iterator find2(const T& key_value, TreeNode<T>* p) {
    ...
    while (p != NULL) {
        ...
        if (key_value > p->value) {p = p->right;}

        // pass left subtree as new tree
        else if (key_value < p->value) {p = p->left;}

        // if the key is found, return p
        else if (key_value == p->value) {return iterator(p);}
    }
