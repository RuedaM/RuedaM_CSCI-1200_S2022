...
template<typename T> unsigned int remove_matching_elements(Vec<T>& vec, const T& t) {
  unsigned int numRemoved = 0;

  if (vec.empty()) {return numRemoved;}

  for (unsigned int i=(vec.size()-1) ; i>0 ; --i) {
    if (vec[i]==t) {
      numRemoved+=1;
      for (unsigned int j=i ; j<vec.size() ; j++) {vec[i] = vec[i+1];}
