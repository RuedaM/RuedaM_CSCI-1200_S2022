...
public:
  ...
  std::pair< iterator, bool > insert(KeyType const& key) {
    ...
    // Using key and hash function, find index from hash value
    unsigned int hash_value = m_hash(key);
    unsigned int index = hash_value % m_table.size();
    // Using index, use find to see if key already exists in hash table
    hash_list_itr p = std::find(m_table[index].begin(), m_table[index].end(), key);

    bool there = false;
    // If not found, note as such and add key
    if (p==m_table[index].end()) {
      there = true;
      m_table[index].push_front(key);
      // Recycle itr p, use to point to added key
      p = m_table[index].begin();
    }
    // Create std::pair to return
    std::pair<iterator, bool> returnPair;
    iterator returnItr(this, index, p);
    returnPair.first = returnItr;
    returnPair.second = there;
    return returnPair;
  }
  ...
  iterator begin() {
    ...
    int index = 0;
    while (index<m_table.size() && m_table[index].size()==0) {++index;}
    // Go to the beginning of the list at the hash table index
    hash_list_itr returnHSItr = m_table[index].begin();
    // Using values, create itr to return
    iterator returnItr(this, index, returnHSItr);
...
private:
  ...
  void resize_table(unsigned int new_size) {
    ...
    std::vector< std::list<KeyType> > new_hash_table(new_size);
    
    // Iterate through old hash_table and copy values
    iterator itr = this->begin();
    while (itr!=this->end()) {
      // Using ptr
      unsigned int hash_value = m_hash(*itr);
      unsigned int index = hash_value % new_size;
      new_hash_table[index].push_back(*itr);
      ++itr;
    }
