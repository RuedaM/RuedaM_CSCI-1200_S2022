/*
cd /mnt/c/users/ruedam/Documents/CSCI1020_DS/Labs/Lab11
g++ -g -Wall -std=c++11 test_ds_hashset.cpp -o test_ds_hashset.out
./test_ds_hashset.out
~/DrMemory-Linux-2.4.0-2/bin64/drmemory -brief -- test_ds_hashset.out
*/
...
public:
  ...
  unsigned int hash1(const std::string& key) {
    unsigned int val = 0;
    for (unsigned int i=0 ; i<key.size() ; ++i) {val += key[i];} // conversion to int is automatic
    return val; // Should mod this value outside of this function
  }
  unsigned int hash2(const std::string& key) {
    unsigned int val = 0;
    for (unsigned int i=0 ; i<key.size() ; ++i) {val = ((val*8)+key[i]);} // conversion to int is automatic
    return val; // Should mod this value outside of this function
  }
};
