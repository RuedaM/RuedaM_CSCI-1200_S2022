/*
cd /mnt/c/users/ruedam/Documents/CSCI1020_DS/Homework/HW08
...
void StudentTests(){
	...
	std::cout<<"|--------BEGIN STUDENT TESTS--------|"<<std::endl;
	Rope test1(IndexTestRope());
	Rope test2(IndexTestRope());
	Rope test3(IndexTestRope());
	std::string testStr1 = "Hello my name is Simon";
	
	std::cout<<"Testing index edges..."<<std::endl;
	char c;
	assert(!test2.index(25,c));
	assert(!test1.index(-1,c));
	assert(test3.index(11,c));
	std::cout<<"Index edges passed"<<std::endl;

	std::cout<<"Testing concat edge..."<<std::endl;
	int heldSize = test1.size() + test3.size();
	test1.concat(test3);
	assert(test1.size() == heldSize);
	std::cout<<"Concat edge passed"<<std::endl;

	std::cout<<"Testing report edge..."<<std::endl;
	std::string testStr2;
	assert(test3.report(0,2,testStr2) && testStr2=="Hel");
	
	std::cout<<"Report edge passed"<<std::endl;
}
...
void BasicTests(){
	...
	/*
	for (rope_iterator itr = r1.begin() ; itr!=r1.end() ; ++itr) {
		if (!is_leaf()) {exit(12);}
