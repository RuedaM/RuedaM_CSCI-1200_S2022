...
public:
	...
	std::string operator*() {return ptr_->value;} //~ADDED~
private:
	Node* ptr_;
};
...
private:
	...
	int size_from_root(Node* p); //~ADDED~ SEE Rope_student.cpp
	Node* get_leaf(int& i) const; //~ADDED~ SEE Rope_student.cpp
	void concat_no_copy(Rope& r); //~ADDED~ SEE Rope_student.cpp
	//Node* copy(Node* oldNode, Node* oldNodeParent); //~ADDED~ SEE Rope_student.cpp
	Node* copy(Node* &oldNode, Node* newNode); //~ADDED~ SEE Rope_student.cpp
	void destroy(Node* p); //~ADDED~ SEE Rope_student.cpp
private:
	int size_;
	Node *root;
};
